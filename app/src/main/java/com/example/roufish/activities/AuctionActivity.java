...
public class AuctionActivity extends AppCompatActivity {
        ...
        query.addSnapshotListener(this, (value, error) -> {
            ...
            for (DocumentSnapshot document : value.getDocuments()) {
                ...
                imageRef.getDownloadUrl().addOnSuccessListener(uri -> {
                    ListLelang lelang = new ListLelang(name, startingPrice, uri.toString(),documentId,description);
                    productList.add(lelang);
                    auctionAdapter.notifyDataSetChanged();
                }).addOnFailureListener(exception -> {
                    // Handle failure (e.g., set a default image URL)
                    ListLelang lelang = new ListLelang(name, startingPrice, "default_image_url",documentId,description);
                    productList.add(lelang);
                });
    ...
    private void openPageLelang(ListLelang product) {
        ...
        intent.putExtra("document_id", product.getDocumentId());
